...
class GroundingDINOWrapper:
    ...
    def load(self):
        """모델 로드"""
        try:
            # HuggingFace transformers 우선 사용 (자동 다운로드)
            ...
            print(f"[Warning] HuggingFace loading failed: {e}")

            # Fallback: groundingdino-py 패키지 시도
            try:
                ...
                raise RuntimeError(f"Failed to load GroundingDINO: {e2}")
    ...
    def _detect_hf(self, image: Image.Image, text_prompt: str) -> List[DetectionResult]:
        ...
        # transformers 4.57+ API: threshold는 model output에서 직접 필터링
        # post_process는 target_sizes만 받음
        ...
        for box, score, label in zip(
            ...
            # Threshold 필터링 (post-process 후에 적용)
