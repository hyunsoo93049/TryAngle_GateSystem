...
class ImprovedMarginAnalyzer:
    ...
    def _analyze_horizontal_balance(self, curr: Dict, ref: Dict) -> Dict:
        ...
        # 1. 완벽: 중심 이동 5% 미만 AND 개별 차이 5% 미만
        if abs(center_shift) < self.thresholds['horizontal']['perfect'] and max_diff < 0.05:
            status = 'perfect'
            score = 95
        # 2. 양호: 중심 이동 10% 미만 AND 개별 차이 10% 미만
        elif abs(center_shift) < self.thresholds['horizontal']['good'] and max_diff < 0.10:
            status = 'good'
            score = 85
        # 3. 조정 필요: 그 외
    ...
    def _analyze_vertical_balance(self, curr: Dict, ref: Dict) -> Dict:
        ...
        # 상태 판정
        # 상태 판정 (개선: 절대 차이도 고려)
        max_diff = max(abs(top_diff), abs(bottom_diff))

        # 1. 완벽: 위치 차이 5% 미만 AND 개별 차이 5% 미만
        if abs(position_diff) < 0.05 and max_diff < 0.05:
            status = 'perfect'
            score = 95
        # 2. 양호: 위치 차이 10% 미만 AND 개별 차이 10% 미만
        elif abs(position_diff) < 0.10 and max_diff < 0.10:  # 10% 이내
            status = 'good'
            score = 85
        # 3. 조정 필요
